...
- **Bonds:** Standard costs $10,000 and pays $500 per five game minutes. Pegged coupon is `(2000 - AERIUM market price) / 10`, including negative yields. Both face values and the otherwise unspecified pegged purchase price default to $10,000; purchases are limited to one lifetime purchase of each type per player. Bonds never mature in-event, so principal is marked at purchase price, not face. A purchase is refused unless free cash stays above the margin-call threshold afterwards.
- **Tax/grant defaults:** Tax ranks human traders by cash, not MtM wealth, and requires a strict majority of votes cast; ties/no votes fail. The current core tax helper uses rounded, minimum-one top/bottom cohorts, which can overlap in small groups. Grant uses core `grantWinner`: only **positive** AERIUM holdings qualify. Settlement supplies user-ID-ordered humans, making equal-positive ties deterministic; all-flat, all-short, or empty cohorts receive no award.
- **Halftime and loans:** Status stays `live` with `frozen = true`; matching/new risk stops but timeline, carry, coupons, and loan deductions continue. Margin liquidation is deferred until reopening. `paused` tears down the runner and is not halftime. Negative-cash humans receive publicly announced rescue loans at 60; funded debt is **2x principal**, repaid in equal installments over remaining game minutes, with residual debt due at event end. Funding remains engine-owned; null `fundedAt` is not proof that cash should be credited.
- **Options and finalization:** Exercise is rejected at or beyond the strict wall-time deadline, including delayed processing. Assignment has a 30-second grace path with a default 20% adverse border price. At finalization, remaining options expire worthless (no automatic exercise), working orders are cancelled, remaining loan debt is deducted, and a final checkpoint is flushed. Final scoring stores snapshots and `finalResults`, publishes rankings, then the runner marks `finalizedAt` and broadcasts: "Trading halted. Final mark-to-market and rankings are complete."
